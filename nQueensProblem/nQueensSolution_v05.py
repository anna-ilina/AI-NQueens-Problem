...
def generateInitialBoard(size):
  ...
  for row in range(size):
    ...
    #bestCol = minConflictsWithinRow(size, queensInCol, queensInDiagonalFromTopRight, queensInDiagonalFromTopLeft, row, None)
    minNumConflicts = size*size + 10 #just some large number
    bestColumns = []
    for col in range(size):
      numConflicts = queensInCol[col] + queensInDiagonalFromTopRight[row + col] + queensInDiagonalFromTopLeft[row + size - col - 1]
      if numConflicts < minNumConflicts:
        ...
        bestColumns = [col]
      elif numConflicts == minNumConflicts:
        bestColumns.append(col)

      #conflictsByCol[col] = queensInCol[col] + queensInDiagonalFromTopRight[row + col] + queensInDiagonalFromTopLeft[row + size - col - 1]

    bestCol = bestColumns[random.randint(0, len(bestColumns) -1)]
    #col = conflictsByCol.index(min(conflictsByCol))
    #print ("row " + str(row) + ". minNumConflicts = " + str(minNumConflicts) + "    cols with minNumConflicts: " + str(bestColumns) + "\t choose col " + str(bestCol))

    board[row] = bestCol
    queensInCol[bestCol] += 1
    queensInDiagonalFromTopRight[row + bestCol] += 1
    queensInDiagonalFromTopLeft[row + size - bestCol - 1] += 1

    # print ("queensInCol: " + str(queensInCol))
    # print ("queensInDiagonalFromTopRight: " + str(queensInDiagonalFromTopRight))
    # print ("queensInDiagonalFromTopLeft: " + str(queensInDiagonalFromTopLeft))
    # print ("")
...
def main():
  ...
  for size in boardSizes:
    ...
    while finalBoard == False:
      numRestarts += 1
      print("generating initial board...")
